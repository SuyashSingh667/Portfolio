...
export default function BlackHole(props: Props) {
    ...
    useEffect(() => {
        ...
        const ro = new ResizeObserver((entries) => {
            for (const entry of entries) {
                ...
                console.log("BlackHole ResizeObserver entry contentRect:", width, height);
                ...
                if (prev.w !== width || prev.h !== height) {
                    console.log("BlackHole triggering size re-seed from version update");
                    setSizeVersion((v) => v + 1)
                }
            }
        })
        console.log("BlackHole ResizeObserver initialized and observing container");
    ...
    useEffect(() => {
        ...
        const draw = (now: number) => {
            ...
            const { w, h } = sizeRef.current
            if (frameCount % 300 === 0) {
                console.log("BlackHole draw loop frame:", frameCount, "dimensions:", w, h, "particles:", particlesRef.current.length);
            }
            const dpr = Math.min(window.devicePixelRatio || 1, 1.5)

...
function renderTextToCanvas(
    ...
    ctx.font = `${fontStyle} ${fontWeight} ${finalSize}px ${fontFamily}, sans-serif`
    const m = ctx.measureText(text)
    const textW = m.width || 1
    const textH = (m.actualBoundingBoxAscent || finalSize * 0.8) + (m.actualBoundingBoxDescent || finalSize * 0.2)
    
    const scale = Math.min(1, maxW / textW, maxH / textH)
    if (scale < 1) {
        finalSize = Math.max(8, finalSize * scale)
    }
    ...
    if (textAlign === "left") {
        ctx.textAlign = "left"
        ctx.fillText(text, width * 0.01, height / 2)
    } else if (textAlign === "right") {
        ctx.textAlign = "right"
        ctx.fillText(text, width * 0.99, height / 2)
    } else {
        ctx.textAlign = "center"
...
export default function MeshText(props: any) {
    ...
    const { text, color, font, colorSplit, customColors, force } = finalProps
    ...
    const textAlign: string = font?.textAlign ?? "left"

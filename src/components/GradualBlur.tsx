...
const useResponsiveDimension = (responsive: boolean, config: any, key: 'height' | 'width') => {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      let v = config[key];
      const keySuffix = key[0].toUpperCase() + key.slice(1);
      const mobileKey = `mobile${keySuffix}`;
      const tabletKey = `tablet${keySuffix}`;
      const desktopKey = `desktop${keySuffix}`;

      if (w <= 480 && config[mobileKey] !== undefined)
        v = config[mobileKey];
      else if (w <= 768 && config[tabletKey] !== undefined)
        v = config[tabletKey];
      else if (w <= 1024 && config[desktopKey] !== undefined)
        v = config[desktopKey];
      setValue(v);
    };
